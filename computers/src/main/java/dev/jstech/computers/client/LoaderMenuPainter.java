 ...
 *
 * <p>Its colours are the palette {@code jsc:screen/loader_menu}.
 */
@PaletteHolder
public final class LoaderMenuPainter {
    ...
    private static final Palette<Colours> PALETTE = Palettes.declare(JsComputers.MODID, "screen/loader_menu",
            new Colours(0xFFBDBDBD, 0xFFFFFFFF, 0xFF000000));
    ...
    public static void draw(final GuiGraphics g, final Font font, final int x, final int y, final BootMenu menu,
        ...
        g.fill(x, y, x + LoaderMenuLayout.WIDTH, y + LoaderMenuLayout.HEIGHT, PALETTE.get().ground());
    ...
    private static void box(final GuiGraphics g, final Font font, final int x, final int y, final String title) {
        ...
        g.fill(left, top, right, top + 1, c.text());
        g.fill(left, bottom - 1, right, bottom, c.text());
        g.fill(left, top, left + 1, bottom, c.text());
        g.fill(right - 1, top, right, bottom, c.text());
        final int tx = x + LoaderMenuLayout.TITLE_X;
        final int ty = y + LoaderMenuLayout.TITLE_Y;
        g.fill(tx - 3, ty - 1, tx + TextWall.width(font, title) + 3, ty + TextWall.ROW, c.ground());
        TextWall.draw(g, font, title, tx, ty, c.bright());
    }
    ...
    private static void entries(final GuiGraphics g, final Font font, final int x, final int y,
        ...
        for (int i = 0; i < menu.entries().size(); i++) {
            ...
            TextWall.draw(g, font, number, left, ty, c.bright());
            TextWall.draw(g, font, name, after, ty, c.text());
            if (i == menu.defaultIndex()) {
                TextWall.draw(g, font, "[Enter]", after + TextWall.width(font, name + " "), ty, c.bright());
            }
    ...
    private static void foot(final GuiGraphics g, final Font font, final int x, final int y, final BootMenu menu,
        ...
        final Colours c = PALETTE.get();
        if (held) {
            TextWall.draw(g, font, GameText.resolve(BootMenu.AUTOBOOT_PAUSED), x, y, c.text());
            return;
        }
        /*
        ...
        TextWall.draw(g, font, before, x, y, c.text());
        TextWall.draw(g, font, seconds, x + TextWall.width(font, before), y, c.bright());
        // Each piece starts where the whole line before it ends, so three roundings never add up to a gap.
        TextWall.draw(g, font, after, x + TextWall.width(font, before + seconds), y, c.text());
    }

    /** The loader's grey, the white it picks its numbers and its keys out in, and the ground under both. */
    private record Colours(int text, int bright, int ground) {
    }
}
