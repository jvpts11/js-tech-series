...
public final class Parser {
     ...
     */
    private IExpr parseHole(final Token token, final dev.jstech.computers.sigma.lex.Lexer.Hole hole) {
        ...
        final String padded = "\n".repeat(Math.max(0, hole.line() - 1)) + " ".repeat(Math.max(0, hole.column() - 1))
                + hole.code();
        final dev.jstech.computers.sigma.lex.Lexer lexer = new dev.jstech.computers.sigma.lex.Lexer(
                new dev.jstech.computers.sigma.SourceFile("", padded), this.diagnostics);
