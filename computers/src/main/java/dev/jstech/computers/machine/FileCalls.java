 ...
 *
 * <p>Nothing here is free: a disk is slower than adding two numbers, and what each call costs is declared beside it.
 */
final class FileCalls {
    ...
    static void bind(final Map<MemberId, MachineCalls.Binding<?>> bindings) {
        file(bindings, "Exists", (files, target, arguments, line) -> files.exists(path(arguments)), STRING);
        file(bindings, "Read", (files, target, arguments, line) -> {
        ...
        file(bindings, "TryRead", (files, target, arguments, line) -> {
            // What was found is filled in beside the answer, and is empty when nothing was.
            final ICliComputer.FsResult read = files.read(path(arguments));
            arguments[1] = read.ok() ? read.message() : "";
            return read.ok();
        }, STRING, "out " + STRING);
        file(bindings, "Write", (files, target, arguments, line) -> files.write(path(arguments), text(arguments)),
                STRING, STRING);
        file(bindings, "Append", (files, target, arguments, line) -> files.append(path(arguments), text(arguments)),
                STRING, STRING);
        file(bindings, "Delete", (files, target, arguments, line) -> files.delete(path(arguments)), STRING);
        file(bindings, "MkDir", (files, target, arguments, line) -> files.makeDir(path(arguments)), STRING);
        file(bindings, "List", (files, target, arguments, line) -> {
