...
public final class SigmaApiGameTests {

    private SigmaApiGameTests() {
    }
    ...
    private static String listing(final String source) {
        ...
        final SigmaCompiler.Result built =
                SigmaCompiler.compile(List.of(new SourceFile("Script.sgs", whole)));
        if (!built.ok()) {
                ...
                .thenExecuteAfter(20, () -> {
                    ...
                    for (final var record : log) {
                        for (final var move : record.moves()) {
                            named = named || move.to().contains("Σ#: Programs.Restock")
                                    || move.from().contains("Σ#: Programs.Restock");
                        }
                    }
    ...
    @GameTest(template = ARENA)
    public static void sgpack_publishesToTheMirrorAndTheNetworkOffersIt(final GameTestHelper helper) {
        final dev.jstech.tests.testkit.TestWorldBuilder.CraftingNetwork wired =
                dev.jstech.tests.testkit.TestWorldBuilder.forGameTest(helper).buildCraftingNetwork();
        final CraftingComputerBlockEntity computer = wired.cc();
        ...
        helper.startSequence()
                .thenExecuteAfter(SETTLE, () -> {
                    final var shell = new dev.jstech.computers.program.ServerCliComputer(
                            computer, helper.getLevel());
                    // A package as it would come off 'sgpack build'.
                    final var manifest = new dev.jstech.computers.sigma.pack.Manifest(
                            "stockwatch", "1.0.0", "jvpts11", "stockwatch.asm", "bell", 1,
                            List.of("stockwatch.asm"), "Tells you when the iron runs low");
                    final var packed = new dev.jstech.computers.sigma.pack.Packed(manifest,
                    ...
                    final var back = dev.jstech.computers.sigma.pack.Packed.read(
