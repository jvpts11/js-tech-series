 ...
 *
 * <p>The explorer names a new file "New File.sgs", with a space in it, and a player working in a
 * folder types the name without the folder. Both have to reach the compiler whole.
 */
@GameTestHolder(JsTests.MODID)
@PrefixGameTestTemplate(false)
public final class SigmaShellGameTests {

    private SigmaShellGameTests() {
    }
    ...
    public static void editors_areCommandsOnceInstalled(final GameTestHelper helper) {
        ...
        helper.startSequence()
                .thenExecuteAfter(SETTLE, () -> {
                    ...
                    final String before = text(shell.run("vim progs/a.sgs", cli));
                    ...
                    final CliShell.Response vim = shell.run("vim progs/a.sgs", cli);
                    ...
                    final CliShell.Response emacs = shell.run("emacs progs/a.sgs", cli);
                     ...
                     */
                    DiskFilesystem.write(computer.systemDisk(), "progs/a.sgs", FileType.SGS, HELLO,
                    ...
                    final CliShell.Response inside = shell.run("vim a.sgs", cli);
                    helper.assertTrue(inside.handOver() != null && "progs/a.sgs".equals(inside.handOver().path()),
                            "a file named from its folder is handed over whole; got "
                                    + (inside.handOver() == null ? "nothing" : inside.handOver().path()));
                    final CliShell.Response dos = shell.run("vim ..\\progs\\a.sgs", cli);
                    helper.assertTrue(dos.handOver() != null && "progs/a.sgs".equals(dos.handOver().path()),
    ...
    public static void dir_listsFilesAndFoldersAlike(final GameTestHelper helper) {
        ...
        helper.startSequence()
                .thenExecuteAfter(SETTLE, () -> {
                    DiskFilesystem.write(computer.systemDisk(), "Projects/Farm/Farm/Program.sgs", FileType.SGS, HELLO,
                            Long.MAX_VALUE, FilesystemKind.HIERARCHICAL);
                    ...
                    helper.assertTrue(inside.contains("Program.sgs"), "the project folder lists its source; got " + inside);
                })
                .thenSucceed();
    }
     ...
     */
    @GameTest(template = ARENA)
    public static void sigma_aProgramReadsALineTypedAtItsTerminal(final GameTestHelper helper) {
        final CraftingComputerBlockEntity computer = computer(helper, new BlockPos(2, 2, 2));
        if (computer == null) {
            return;
        }
        ...
        helper.startSequence()
                .thenExecuteAfter(SETTLE, () -> {
                    ...
                    final String ran = text(shell.run("sigma run progs/asks.asm", cli));
                    helper.assertFalse(ran.contains("not found") || ran.contains("No such") || ran.contains("sigma:"),
     ...
     */
    @GameTest(template = ARENA)
    public static void sigma_aProgramWaitingForALineWithNoTerminalIsStopped(final GameTestHelper helper) {
        final CraftingComputerBlockEntity computer = computer(helper, new BlockPos(2, 2, 2));
        if (computer == null) {
            return;
        }
     ...
     */
    @GameTest(template = ARENA)
    public static void sigma_listsAProgramByItsOwnNameOrByTheRuntime(final GameTestHelper helper) {
        final CraftingComputerBlockEntity computer = computer(helper, new BlockPos(2, 2, 2));
        if (computer == null) {
            return;
        }
        ...
        helper.startSequence()
                .thenExecuteAfter(SETTLE, () -> {
                    DiskFilesystem.write(computer.systemDisk(), "progs/named.sgs", FileType.SGS, NAMED,
                            Long.MAX_VALUE, FilesystemKind.HIERARCHICAL);
                    DiskFilesystem.write(computer.systemDisk(), "progs/asks.sgs", FileType.SGS, ASKS,
                            Long.MAX_VALUE, FilesystemKind.HIERARCHICAL);
                    final ServerCliComputer cli = new ServerCliComputer(computer, helper.getLevel());
                    final CliShell shell = CliCommands.shellFor(cli, 52);
                    helper.assertTrue(text(shell.run("sgsc progs/named.sgs", cli)).contains("wrote"), "named compiles");
                    helper.assertTrue(text(shell.run("sgsc progs/asks.sgs", cli)).contains("wrote"), "asks compiles");
                    shell.run("sigma run progs/named.asm", cli);
                    shell.run("sigma run progs/asks.asm", cli);
                })
                .thenExecuteAfter(SETTLE * 4, () -> {
                    final ServerCliComputer cli = new ServerCliComputer(computer, helper.getLevel());
                    final CliShell shell = CliCommands.shellFor(cli, 52);
                    final String listed = text(shell.run("sigma ps", cli));
    ...
    @GameTest(template = ARENA)
    public static void sgsc_compilesANameWithASpaceFromTheRootAndFromItsFolder(final GameTestHelper helper) {
        final CraftingComputerBlockEntity computer = computer(helper, new BlockPos(2, 2, 2));
        if (computer == null) {
            return;
        }
        helper.startSequence()
                .thenExecuteAfter(SETTLE, () -> {
                    DiskFilesystem.write(computer.systemDisk(), "progs/New File.sgs", FileType.SGS, HELLO,
                            Long.MAX_VALUE, FilesystemKind.HIERARCHICAL);
                    final ServerCliComputer cli = new ServerCliComputer(computer, helper.getLevel());
                    final CliShell shell = CliCommands.shellFor(cli, 52);
                    final String fromRoot = text(shell.run("sgsc \"progs/New File.sgs\"", cli));
                    ...
                    final String fromFolder = text(shell.run("sgsc \"New File.sgs\"", cli));
                    ...
                    final String unquoted = text(shell.run("sgsc New File.sgs", cli));
                    helper.assertTrue(unquoted.contains("sgsc:") && !unquoted.contains("wrote"),
                            "two words are two files, and the first is not there; got " + unquoted);
                    final String ran = text(shell.run("sigma run \"New File.asm\"", cli));
