    ...
    public static void parts_allComeBackFromTheComputersTag(final GameTestHelper helper) {
        ...
        helper.startSequence()
                .thenExecuteAfter(SETTLE, () -> {
                    final MachinePrograms.Started started = pc.cannon().start("counter.can", COUNTER, 1, pc);
                    helper.assertTrue(started.ok(), "the counter starts: " + started.message());
                    for (int i = 0; i < 3; i++) {
                        pc.cannon().tick(PLENTY);
                    }
                    final List<String> said = pc.cannon().byId(started.id()).process().console();
                    ...
                    helper.assertTrue(fresh.cannon().all().size() == 1, "the program comes back; got "
                            + fresh.cannon().all().size());
                    helper.assertTrue(fresh.cannon().all().getFirst().process().console().equals(said),
                            "with what it had said; got " + fresh.cannon().all().getFirst().process().console());
