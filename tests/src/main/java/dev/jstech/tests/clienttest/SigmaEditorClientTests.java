 ...
 */
public final class SigmaEditorClientTests {

    private SigmaEditorClientTests() {
    }
    ...
    public static void virtualStudioCode_writesAProgramTheMachineThenRuns(final ClientTestContext ctx) {
        ctx.thenBuild(0, world -> {
                    ...
                    for (final String id : new String[] {"virtual_studio_code", "sgsc", "sigma"}) {
                 ...
                 */
                .then(SETTLE, () -> editor(ctx).openFile("progs/hello.sgs"))
                .thenWaitUntil(() -> editor(ctx).openFile().equals("progs/hello.sgs"),
                 ...
                 */
                .then(SETTLE, () -> editor(ctx).runInTerminal("sgsc progs/hello.sgs"))
                ...
                .then(SETTLE, () -> editor(ctx).runInTerminal("sigma run progs/hello.asm"))
    ...
    private static final String FARM_SILO = FARM_PROJECT + "/Silo.sgs";
    ...
    private static void seedFarm(final CraftingComputerBlockEntity computer) {
        ...
        seed(computer, FARM_SILO, FileType.SGS, "namespace Farm; class Silo { public long Stored; }");
    }
    ...
    private static ClientTestContext studioOnFarm(final ClientTestContext ctx) {
        return ctx.thenBuild(0, world -> {
                    final CraftingComputerBlockEntity computer = world.placeRunningCraftingComputer(COMPUTER);
                    computer.installOs(FRAMES_XP);
                    for (final String id : new String[] {"virtual_studio", "sgsc", "sigma"}) {
                        computer.console().install(program(id).toString());
                    }
                    seedFarm(computer);
                    world.placeMonitor(MONITOR, Direction.EAST);
                ...
                .thenWaitUntil(() -> "Farm".equals(studio(ctx).solutionName())
                                && studio(ctx).explorerLabels().contains("Silo.sgs"),
    ...
    public static void virtualStudio_deletesAndAddsSourcesFromTheTree(final ClientTestContext ctx) {
        studioOnFarm(ctx)
                .then(SETTLE, () -> ctx.rightClickDesktop(studio(ctx).explorerRowPoint("Silo.sgs")))
                ...
                .thenWaitUntilServer(level -> !onDisk(ctx, level, FARM_SILO)
                                && !diskText(ctx, level, FARM_PROJECT + "/Farm.sgsproj").contains("Silo.sgs"),
                ...
                .thenWaitUntil(() -> !studio(ctx).explorerLabels().contains("Silo.sgs")
                ...
                .then(SETTLE, () -> studio(ctx).answerAsk("Barn.sgs"))
                .thenWaitUntil(() -> (FARM_PROJECT + "/Barn.sgs").equals(studio(ctx).openFile()), SCREEN_WAIT,
                        "the new item to be on a tab")
                .then(0, () -> ctx.type("namespace Farm; class Barn { }"))
                .then(0, () -> ctx.key(GLFW.GLFW_KEY_S, GLFW.GLFW_MOD_CONTROL))
                .thenWaitUntilServer(level -> onDisk(ctx, level, FARM_PROJECT + "/Barn.sgs")
                                && diskText(ctx, level, FARM_PROJECT + "/Farm.sgsproj").contains("Barn.sgs"),
                        SCREEN_WAIT, "the new item to be on the disk and in the project file",
                        level -> "project=" + diskText(ctx, level, FARM_PROJECT + "/Farm.sgsproj"))
                .thenWaitUntil(() -> studio(ctx).explorerLabels().contains("Barn.sgs"), SCREEN_WAIT,
    ...
    public static void virtualStudioCode_opensOnADoubleClickAndKeepsItsTabs(final ClientTestContext ctx) {
        ctx.thenBuild(0, world -> {
                    ...
                    seed(computer, "progs/hello.sgs", FileType.SGS, SOURCE);
                    world.placeMonitor(MONITOR, Direction.EAST);
                })
                .thenTeleport(SETTLE, PLAYER_AT_MONITOR, Direction.WEST)
                .thenRightClick(SETTLE, MONITOR)
                ...
                .thenWaitUntil(() -> editor(ctx) != null, SCREEN_WAIT, "the editor window to open")
                .then(SETTLE, () -> editor(ctx).openFolder("progs"))
                .thenWaitUntil(() -> editor(ctx).sideLabels().contains("hello.sgs"), SCREEN_WAIT,
                ...
                .then(SETTLE, () -> ctx.clickDesktop(editor(ctx).sideRowPoint("hello.sgs")))
                ...
                .then(SETTLE, () -> {
                    final int[] row = editor(ctx).sideRowPoint("hello.sgs");
                    ctx.clickDesktop(row);
                    ctx.clickDesktop(row);
                })
                .thenWaitUntil(() -> "progs/hello.sgs".equals(editor(ctx).openFile()), SCREEN_WAIT,
                ...
                .thenAssert(SETTLE, () -> "progs/hello.sgs".equals(editor(ctx).openFile()),
    ...
    public static void virtualStudioCode_suggestsWhatTheProgramItselfDeclares(final ClientTestContext ctx) {
        ctx.thenBuild(0, world -> {
                ...
                .thenWaitUntil(() -> editor(ctx) != null, SCREEN_WAIT, "the editor window to open")
                .then(SETTLE, () -> editor(ctx).openFolder("progs"))
                .then(SETTLE, () -> editor(ctx).openFile("progs/silo.sgs"))
                .thenWaitUntil(() -> editor(ctx).openFile().equals("progs/silo.sgs"),
                        SCREEN_WAIT, "the editor to open the first file")
                .then(0, () -> ctx.type(SILO))
                .then(0, () -> ctx.key(GLFW.GLFW_KEY_S, GLFW.GLFW_MOD_CONTROL))
                .thenWaitUntilServer(level -> onDisk(ctx, level, "progs/silo.sgs"),
                        SCREEN_WAIT, "the first file to be on the machine's disk", level -> "")
                .then(SETTLE, () -> editor(ctx).openFile("progs/farm.sgs"))
                .thenWaitUntil(() -> editor(ctx).openFile().equals("progs/farm.sgs"),
    ...
    public static void virtualStudio_createsASolutionBuildsItAndStartsIt(final ClientTestContext ctx) {
        ctx.thenBuild(0, world -> {
                ...
                .thenWaitUntil(() -> studio(ctx).openFile().equals("progs/Hello/Hello/Hello.sgs"),
                ...
                .thenWaitUntilServer(level -> computerOf(ctx, level).sigma().all().isEmpty(), SCREEN_WAIT,
                        "the finished program to leave the machine's process list",
                        level -> "still listed: " + computerOf(ctx, level).sigma().all().stream()
