...
- Inside the mods, with no change to how anything behaves: the handlers behind the computers' screens and
  ...
  keeps who it is, its console, its random numbers, the lines typed at it and the calls waiting their turn in
  parts of their own, and its heap takes in whatever the machine hands the program and checks what the program
  reaches into; every source file's header names the mod it belongs to.
