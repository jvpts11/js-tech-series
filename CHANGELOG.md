...
- Inside the mods, with no change to how anything behaves: the handlers behind the computers' screens and
  ...
  programs, the calls on its windows and widgets, the values it reads of itself and of the language's own text and
  collections, and the lists and maps it makes being answered through them; every source file's header names the mod
  it belongs to.
