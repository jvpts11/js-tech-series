...
- A program's calls are matched to the system's declarations when it loads, and answered through them: its console,
  ...
  reads of the computer itself, its network and its Mainframe, and the Operations it asks the network for, which
  services the computer keeps answer. A computer keeps the shell its programs reach its drives and its network
  through, instead of making one for each call.
